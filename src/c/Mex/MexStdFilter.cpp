...
void MexStdFilter::execute(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) const
{
	...
	{
		MexStdFilter_run<bool>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint8(prhs[0]))
	{
		MexStdFilter_run<uint8_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint16(prhs[0]))
	{
		MexStdFilter_run<uint16_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsInt16(prhs[0]))
	{
		MexStdFilter_run<int16_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint32(prhs[0]))
	{
		MexStdFilter_run<uint32_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsInt32(prhs[0]))
	{
		MexStdFilter_run<int32_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsSingle(prhs[0]))
	{
		MexStdFilter_run<float>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsDouble(prhs[0]))
	{
		MexStdFilter_run<double>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else
	{
...
{
	helpLines.push_back("This will take the standard deviation of the given neighborhood.");
