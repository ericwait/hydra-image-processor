...
template <typename T>
void MexMaxFilter_run(const mxArray* inIm, mxArray** outIm, ImageContainer<float> kernel, int numIterations, int device)
{
	T* imageInPtr;
	T* imageOutPtr;
...
void MexMaxFilter::execute( int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[] ) const
{
	int device = -1;
	int numIterations = 1;
	...
	{
		MexMaxFilter_run<bool>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint8(prhs[0]))
	{
		MexMaxFilter_run<uint8_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint16(prhs[0]))
	{
		MexMaxFilter_run<uint16_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsInt16(prhs[0]))
	{
		MexMaxFilter_run<int16_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint32(prhs[0]))
	{
		MexMaxFilter_run<uint32_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsInt32(prhs[0]))
	{
		MexMaxFilter_run<int32_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsSingle(prhs[0]))
	{
		MexMaxFilter_run<float>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsDouble(prhs[0]))
	{
		MexMaxFilter_run<double>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else
	{
...
void MexMaxFilter::usage(std::vector<std::string>& outArgs,std::vector<std::string>& inArgs) const
{
...
void MexMaxFilter::help(std::vector<std::string>& helpLines) const
{
