...
template <typename T>
void MexMinFilter_run(const mxArray* inIm, mxArray** outIm, ImageContainer<float> kernel, int numIterations, int device)
{
	T* imageInPtr;
	T* imageOutPtr;
...
void MexMinFilter::execute(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) const
{
	int device = -1;
	int numIterations = 1;
	...
	{
		MexMinFilter_run<bool>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint8(prhs[0]))
	{
		MexMinFilter_run<uint8_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint16(prhs[0]))
	{
		MexMinFilter_run<uint16_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsInt16(prhs[0]))
	{
		MexMinFilter_run<int16_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsUint32(prhs[0]))
	{
		MexMinFilter_run<uint32_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsInt32(prhs[0]))
	{
		MexMinFilter_run<int32_t>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsSingle(prhs[0]))
	{
		MexMinFilter_run<float>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else if (mxIsDouble(prhs[0]))
	{
		MexMinFilter_run<double>(prhs[0], &plhs[0], kernel, numIterations, device);
	}
	else
	{
...
std::string MexMinFilter::check(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) const
{
	if (nrhs != 4)
...
void MexMinFilter::usage(std::vector<std::string>& outArgs, std::vector<std::string>& inArgs) const
{
...
void MexMinFilter::help(std::vector<std::string>& helpLines) const
{
	helpLines.push_back("This will set each pixel/voxel to the max value of the neighborhood defined by the given kernel.");
