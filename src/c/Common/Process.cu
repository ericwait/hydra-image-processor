...
// }
// 
// void set(Vec<size_t> imageDims, int device=0)
// {
// 	if (g_cudaBuffer==NULL)
// 		g_cudaBuffer = new CudaProcessBuffer<HostPixelType>(imageDims,device);
// }
// 
// void set2(Vec<size_t> imageDims, int device=0)
// {
// 	if (g_cudaBuffer2==NULL)
...
// }
// 
//  void meanFilter( const ImageContainer* imageIn, ImageContainer* imageOut, Vec<size_t> neighborhood )
//  {
// 	 CudaImageContainerClean* cudaImageIn = new CudaImageContainerClean(imageIn);
// 	 CudaImageContainerClean* cudaImageOut = new CudaImageContainerClean(imageIn->getDims());
// 
// 	cudaMeanFilter<<<blocks,threads>>>(cudaImageIn,cudaImageOut,neighborhood);
// 
// #ifdef _DEBUG
...
// #endif // _DEBUG
// 
// 	HANDLE_ERROR(cudaMemcpy((void*)cudaImageOut->getConstImagePointer(),imageOut->getMemoryPointer(),
// 		sizeof(DevicePixelType)*imageOut->getDims().product(),cudaMemcpyDeviceToHost));
// 
// 	delete cudaImageIn;
// 	delete cudaImageOut;
//  }
