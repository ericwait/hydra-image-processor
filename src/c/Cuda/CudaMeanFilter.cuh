...
__global__ void cudaMeanFilter(CudaImageContainer<PixelTypeIn> imageIn, CudaImageContainer<PixelTypeOut> imageOut, Kernel constKernelMem, PixelTypeOut minValue, PixelTypeOut maxValue)
{
...
template <class PixelTypeIn, class PixelTypeOut>
void cMeanFilter(ImageContainer<PixelTypeIn> imageIn, ImageContainer<PixelTypeOut>& imageOut,	ImageContainer<float> kernel, int numIterations = 1, int device = -1)
{
...
#	pragma omp parallel default(shared)
	{
		...
		for (int i = CUDA_IDX; i < chunks.size(); i += N_THREADS)
		{
			...
			{
				cudaMeanFilter <<<chunks[i].blocks, chunks[i].threads>>>(*(deviceImages.getCurBuffer()),*(deviceImages.getNextBuffer()), constKernelMem, MIN_VAL, MAX_VAL);
