...
PyObject* PyWrapOpener::execute(PyObject* self, PyObject* args)
{
	...
	{
		PyWrapOpener_run<bool>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT8 )
	{
		PyWrapOpener_run<uint8_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT16 )
	{
		PyWrapOpener_run<uint16_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_INT16 )
	{
		PyWrapOpener_run<int16_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT32 )
	{
		PyWrapOpener_run<uint32_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_INT32 )
	{
		PyWrapOpener_run<int32_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_FLOAT )
	{
		PyWrapOpener_run<float>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_DOUBLE )
	{
		PyWrapOpener_run<double>(imIn, &imOut, kernel, numIterations, device);
	}
	else
	{
