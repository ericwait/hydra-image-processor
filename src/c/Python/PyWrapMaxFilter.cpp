...
template <typename T>
void PyWrapMaxFilter_run(const PyArrayObject* inIm, PyArrayObject** outIm, ImageContainer<float> kernel, int numIterations, int device)
{
	T* imageInPtr;
	T* imageOutPtr;
...
PyObject* PyWrapMaxFilter::execute(PyObject* self, PyObject* args)
{
	...
	{
		PyWrapMaxFilter_run<bool>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_UINT8 )
	{
		PyWrapMaxFilter_run<uint8_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_UINT16 )
	{
		PyWrapMaxFilter_run<uint16_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_INT16 )
	{
		PyWrapMaxFilter_run<int16_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_UINT32 )
	{
		PyWrapMaxFilter_run<uint32_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_INT32 )
	{
		PyWrapMaxFilter_run<int32_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_FLOAT )
	{
		PyWrapMaxFilter_run<float>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_DOUBLE )
	{
		PyWrapMaxFilter_run<double>(imContig, &imOut, kernel, numIterations, device);
	}
	else
	{
