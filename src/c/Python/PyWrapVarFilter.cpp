...
template <typename T>
void PyWrapVarFilter_run(const PyArrayObject* inIm, PyArrayObject** outIm, ImageContainer<float> kernel, int numIterations, int device)
{
	T* imageInPtr;
	T* imageOutPtr;
...
PyObject* PyWrapVarFilter::execute(PyObject* self, PyObject* args)
{
	...
	{
		PyWrapVarFilter_run<bool>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_UINT8 )
	{
		PyWrapVarFilter_run<uint8_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_UINT16 )
	{
		PyWrapVarFilter_run<uint16_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_INT16 )
	{
		PyWrapVarFilter_run<int16_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_UINT32 )
	{
		PyWrapVarFilter_run<uint32_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_INT32 )
	{
		PyWrapVarFilter_run<int32_t>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_FLOAT )
	{
		PyWrapVarFilter_run<float>(imContig, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imContig) == NPY_DOUBLE )
	{
		PyWrapVarFilter_run<double>(imContig, &imOut, kernel, numIterations, device);
	}
	else
	{
