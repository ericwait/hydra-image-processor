...
PyObject* PyWrapMeanFilter::execute(PyObject* self, PyObject* args)
{
	...
	{
		PyWrapMeanFilter_run<bool>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT8 )
	{
		PyWrapMeanFilter_run<uint8_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT16 )
	{
		PyWrapMeanFilter_run<uint16_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_INT16 )
	{
		PyWrapMeanFilter_run<int16_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT32 )
	{
		PyWrapMeanFilter_run<uint32_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_INT32 )
	{
		PyWrapMeanFilter_run<int32_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_FLOAT )
	{
		PyWrapMeanFilter_run<float>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_DOUBLE )
	{
		PyWrapMeanFilter_run<double>(imIn, &imOut, kernel, numIterations, device);
	}
	else
	{
