...
template <typename T>
void PyWrapMedianFilter_run(const PyArrayObject* inIm, PyArrayObject** outIm, ImageView<float> kernel, int numIterations, int device)
{
...
PyObject* PyWrapMedianFilter::execute(PyObject* self, PyObject* args)
{
	...
	{
		PyWrapMedianFilter_run<bool>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT8 )
	{
		PyWrapMedianFilter_run<uint8_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT16 )
	{
		PyWrapMedianFilter_run<uint16_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_INT16 )
	{
		PyWrapMedianFilter_run<int16_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_UINT32 )
	{
		PyWrapMedianFilter_run<uint32_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_INT32 )
	{
		PyWrapMedianFilter_run<int32_t>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_FLOAT )
	{
		PyWrapMedianFilter_run<float>(imIn, &imOut, kernel, numIterations, device);
	}
	else if ( PyArray_TYPE(imIn) == NPY_DOUBLE )
	{
		PyWrapMedianFilter_run<double>(imIn, &imOut, kernel, numIterations, device);
	}
	else
	{
