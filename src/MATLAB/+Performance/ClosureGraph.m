function times = ClosureGraph(sizes_rc,sizeItter,types,typeItter,numTrials,numDevices)
    ...
    times = ones(length(sizeItter),7,length(typeItter))*inf;
    prgs = Utils.CmdlnProgress(numItters,true,'ClosureFilter');
    k = 0;
    kernel = ones(5,5,3);
    m = memory;
    ...
    for i = sizeItter
        for ty = typeItter
            ...
            if (imSize<=cpu_memAvail && imSizePerFrame<=gpu_memAvail)
                ...
                for j=1:numTrials
                    [ts(j,1),ts(j,2),~,ts(j,3:4)] = Performance.Closure(im,kernel,numDevices);
                    k = k +1;
    ...
    Performance.PlotResults(times,'Closure [5,5,3]',numDevices);
end