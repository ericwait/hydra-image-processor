function times = EntropyFilterGraph(sizes_rc,sizeItter,types,typeItter,numTrials)
    ...
    prgs = Utils.CmdlnProgress(numItters,true,'EntropyFilter');
    j = 0;
    for ty = typeItter
        for i = sizeItter
            if (strcmpi(types{ty},'single'))
                continue
            end
    ...
    Performance.PlotResults(times,'Entropy Filter');
end