...
%    arrayOut = ImProc.MedianFilter(arrayIn,kernel,[numIterations],[device])
...
function arrayOut = MedianFilter(arrayIn,kernel,numIterations,device)
    try
        arrayOut = ImProc.Cuda.MedianFilter(arrayIn,kernel,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.MedianFilter(arrayIn,kernel,numIterations,device);
    end
end
