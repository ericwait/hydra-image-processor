...
%    arrayOut = ImProc.LoG(arrayIn,sigmas,[numIterations],[device])
...
function arrayOut = LoG(arrayIn,sigmas,numIterations,device)
    try
        arrayOut = ImProc.Cuda.LoG(arrayIn,sigmas,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.LoG(arrayIn,sigmas,numIterations,device);
    end
end
