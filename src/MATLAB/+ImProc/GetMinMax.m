...
%    [minValue,maxValue] = ImProc.GetMinMax(arrayIn,[device])
...
function [minValue,maxValue] = GetMinMax(arrayIn,device)
    try
        [minValue,maxValue] = ImProc.Cuda.GetMinMax(arrayIn,device);
    catch errMsg
        warning(errMsg.message);
        [minValue,maxValue] = ImProc.Local.GetMinMax(arrayIn,device);
    end
end
