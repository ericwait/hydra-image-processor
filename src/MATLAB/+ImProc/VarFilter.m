...
%    arrayOut = ImProc.VarFilter(arrayIn,kernel,[numIterations],[device])
...
function arrayOut = VarFilter(arrayIn,kernel,numIterations,device)
    try
        arrayOut = ImProc.Cuda.VarFilter(arrayIn,kernel,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.VarFilter(arrayIn,kernel,numIterations,device);
    end
end
