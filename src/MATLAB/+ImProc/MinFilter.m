...
%    arrayOut = ImProc.MinFilter(arrayIn,kernel,[numIterations],[device])
...
function arrayOut = MinFilter(arrayIn,kernel,numIterations,device)
    try
        arrayOut = ImProc.Cuda.MinFilter(arrayIn,kernel,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.MinFilter(arrayIn,kernel,numIterations,device);
    end
end
