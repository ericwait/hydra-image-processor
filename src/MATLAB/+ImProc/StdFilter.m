...
%    arrayOut = ImProc.StdFilter(arrayIn,kernel,[numIterations],[device])
...
function arrayOut = StdFilter(arrayIn,kernel,numIterations,device)
    try
        arrayOut = ImProc.Cuda.StdFilter(arrayIn,kernel,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.StdFilter(arrayIn,kernel,numIterations,device);
    end
end
