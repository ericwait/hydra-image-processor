...
%    arrayOut = ImProc.EntropyFilter(arrayIn,kernel,[device])
...
function arrayOut = EntropyFilter(arrayIn,kernel,device)
    try
        arrayOut = ImProc.Cuda.EntropyFilter(arrayIn,kernel,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.EntropyFilter(arrayIn,kernel,device);
    end
end
