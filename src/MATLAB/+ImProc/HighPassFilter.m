...
%    arrayOut = ImProc.HighPassFilter(arrayIn,sigmas,[device])
...
function arrayOut = HighPassFilter(arrayIn,sigmas,device)
    try
        arrayOut = ImProc.Cuda.HighPassFilter(arrayIn,sigmas,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.HighPassFilter(arrayIn,sigmas,device);
    end
end
