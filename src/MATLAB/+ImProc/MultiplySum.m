...
%    arrayOut = ImProc.MultiplySum(arrayIn,kernel,[numIterations],[device])
...
function arrayOut = MultiplySum(arrayIn,kernel,numIterations,device)
    try
        arrayOut = ImProc.Cuda.MultiplySum(arrayIn,kernel,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.MultiplySum(arrayIn,kernel,numIterations,device);
    end
end
