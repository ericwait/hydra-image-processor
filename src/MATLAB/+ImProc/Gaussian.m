...
%    arrayOut = ImProc.Gaussian(arrayIn,sigmas,[numIterations],[device])
...
function arrayOut = Gaussian(arrayIn,sigmas,numIterations,device)
    try
        arrayOut = ImProc.Cuda.Gaussian(arrayIn,sigmas,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.Gaussian(arrayIn,sigmas,numIterations,device);
    end
end
