...
%    arrayOut = ImProc.Opener(arrayIn,kernel,[numIterations],[device])
...
function arrayOut = Opener(arrayIn,kernel,numIterations,device)
    try
        arrayOut = ImProc.Cuda.Opener(arrayIn,kernel,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.Opener(arrayIn,kernel,numIterations,device);
    end
end
