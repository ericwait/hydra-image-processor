...
%    arrayOut = ImProc.WienerFilter(arrayIn,[kernel],[noiseVariance],[device])
...
function arrayOut = WienerFilter(arrayIn,kernel,noiseVariance,device)
    try
        arrayOut = ImProc.Cuda.WienerFilter(arrayIn,kernel,noiseVariance,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.WienerFilter(arrayIn,kernel,noiseVariance,device);
    end
end
