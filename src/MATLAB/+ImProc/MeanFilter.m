...
%    arrayOut = ImProc.MeanFilter(arrayIn,kernel,[numIterations],[device])
...
function arrayOut = MeanFilter(arrayIn,kernel,numIterations,device)
    try
        arrayOut = ImProc.Cuda.MeanFilter(arrayIn,kernel,numIterations,device);
    catch errMsg
        warning(errMsg.message);
        arrayOut = ImProc.Local.MeanFilter(arrayIn,kernel,numIterations,device);
    end
end
